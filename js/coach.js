...
const Coach = {
  ...
  async envoyerMessage(question) {
    ...
    // ✅ Vérifier si question liée au programme IA
    if (question.toLowerCase().includes('génère') ||
      ...
      return `Je peux générer un programme personnalisé pour toi ${nom} ! 🧠\n\nVa dans l'onglet **Programme IA** pour répondre au questionnaire et obtenir ton programme sur mesure. Je l'adapterai selon :\n  ✅ Ton objectif\n  ✅ Ton niveau\n  ✅ Tes disponibilités\n  ✅ Ton équipement\n\nClique sur 🧠 Programme Adaptatif dans le menu !`;
    }
  ...
  _raisonnerIA(q, ctx, prs = {}) {
    ...
    if (q.includes('pr') || q.includes('record') || q.includes('max') || q.includes('meilleur')) {
    ...
    if (q.includes('fatigu') || q.includes('récup') || q.includes('repos') || q.includes('douleur') || q.includes('mal') || q.includes('épuisé')) {
    ...
    if (q.includes('programme') || q.includes('plan') || q.includes('séance') || q.includes('aujourd') || q.includes('faire') || q.includes('entraîn')) {
      const phaseMsg = {
    ...
    if (q.includes('manger') || q.includes('nutrition') || q.includes('protéine') || q.includes('calorie') || q.includes('régime') || q.includes('nourriture')) {
      let poids = 80;
    ...
    if (q.includes('motiv') || q.includes('envie') || q.includes('abandon') || q.includes('dur') || q.includes('difficile') || q.includes('arrêt')) {
      return Utils.random([
    ...
    if (q.includes('bonjour') || q.includes('salut') || q.includes('hello') || q.includes('hey') || q.includes('coucou') || q.includes('bonsoir')) {
  ...
  // ════════════════════════════════════════════════════════
  // ✅ PROGRAMME IA — GÉNÉRATEUR COMPLET
  // ════════════════════════════════════════════════════════
  ProgrammeIA: {
    ...
    generer(reponses) {
      ...
      // ✅ Appliquer le planning dans l'app
    ...
    // ✅ GÉNÉRATION SÉANCES
    // ════════════════════════════════════════════════════
    _genererSeances(style, objectif, niveau, nbJours, duree, equipement) {

      // Paramètres selon niveau
      const params = {
        debutant:      { series:3, repsForce:'8-10', repsVol:'12-15', repsEnd:'15-20', repos:75  },
        intermediaire: { series:4, repsForce:'5-8',  repsVol:'8-12',  repsEnd:'12-15', repos:90  },
        avance:        { series:5, repsForce:'3-6',  repsVol:'6-10',  repsEnd:'10-15', repos:120 }
      ...
      if (style === 'full_body') {
        return this._seancesFullBody(objectif, niveau, nbJours, s, reps, r, duree, equipement);
      }
      if (style === 'upper_lower') {
        return this._seancesUpperLower(objectif, niveau, nbJours, s, reps, r, duree, equipement);
      }
      // PPL par défaut
      return this._seancesPPL(objectif, niveau, nbJours, s, reps, r, duree, equipement);
    },

    // ════════════════════════════════════════════════════
    // ✅ PUSH / PULL / LEGS
    // ════════════════════════════════════════════════════
    _seancesPPL(objectif, niveau, nbJours, s, reps, r, duree, equip) {
      const push = {
        id:            'ia_push',
      ...
      // Rotation PPL selon nb jours
      if (nbJours === 3)  return [push, pull, legs];
      if (nbJours === 4)  return [push, pull, legs,
        { ...push, id:'ia_push2', nom:'Push B — Variation' }];
      if (nbJours === 5)  return [push, pull, legs, push, pull];
      return [push, pull, legs, push, pull, legs]; // 6 jours
    },

    // ════════════════════════════════════════════════════
    // ✅ FULL BODY
    // ════════════════════════════════════════════════════
    _seancesFullBody(objectif, niveau, nbJours, s, reps, r, duree, equip) {
      const fbA = {
        id:            'ia_fba',
        nom:           'Full Body A',
        emoji:         '🔄',
    ...
    // ✅ UPPER / LOWER
    // ════════════════════════════════════════════════════
    _seancesUpperLower(objectif, niveau, nbJours, s, reps, r, duree, equip) {
      const upper = {
        id:            'ia_upper',
    ...
    _filtrerExos(exercices, equipement, duree) {
      const maxExos = duree <= 45 ? 4
                    : duree <= 60 ? 5
                    : duree <= 75 ? 6
                    : 7;

      // Mapping équipement → exercices requis
    ...
    _genererPlanning(seances, nbJours, joursSpecifiques) {
      ...
      // Si pas de jours spécifiques → distribuer intelligemment
      if (!joursActifs || joursActifs.length === 0) {
        const distributions = {
          3: [0, 2, 4],           // Lun / Mer / Ven
          4: [0, 1, 3, 4],        // Lun / Mar / Jeu / Ven
          5: [0, 1, 2, 3, 4],     // Lun à Ven
          6: [0, 1, 2, 3, 4, 5]   // Lun à Sam
        };
    ...
    _appliquerPlanning(planning) {
      try {
        const nouveauPlanning = planning.map(j => ({
          jour:     j.jour,
          label:    j.label,
          seanceId: j.seanceId
        }));
        ...
        console.warn('[ProgrammeIA] Erreur application planning:', e);
      }
    },
    ...
    _renderQuestionnaire(container) {
      ...
      container.innerHTML = `
        ...
        <div style="display:grid;
                    grid-template-columns:${this._etapeActuelle > 0 ? '1fr 2fr' : '1fr'};
                    gap:8px">
    ...
    _renderOptionsEtape(etape) {
      ...
      if (etape.type === 'jours_semaine') {
        const reps = this._reponses[etape.id] || [];
        const jours = [
          {idx:0,label:'Lun'},{idx:1,label:'Mar'},{idx:2,label:'Mer'},
          {idx:3,label:'Jeu'},{idx:4,label:'Ven'},{idx:5,label:'Sam'},
          {idx:6,label:'Dim'}
        ];
        return `
          <div style="display:grid;grid-template-columns:repeat(7,1fr);
            ...
            ${jours.map(j => {
              ...
              return `
                <button onclick="Coach.ProgrammeIA._toggleJour(${j.idx},this)"
    ...
    _selectionnerChoix(etapeId, val, btn) {
      this._reponses[etapeId] = val;

      // Mettre à jour visuellement
      const parent = btn.parentElement;
      parent.querySelectorAll('[data-val]').forEach(b => {
        const isActif = b.dataset.val === val;
        b.style.background   = isActif ? 'rgba(75,75,249,0.2)' : 'var(--bg-input)';
        b.style.borderColor  = isActif ? 'var(--fd-indigo)' : 'var(--border-color)';
      });
    },
    ...
    _lancerGeneration() {
      ...
      setTimeout(() => {
        try {
          ...
          this._etapeActuelle     = 0;
          this._renderProgrammeGenere(container, programme);
    ...
    _renderProgrammeGenere(container, prog) {
      ...
      container.innerHTML = `
        ...
        <div class="card mb-md"
          ...
          <div style="font-size:.72rem;color:var(--text-muted)">
            Généré le ${Utils.formatDateLong(prog.dateCreation)}
          </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;
          ...
          <button onclick="Coach.ProgrammeIA._modeQuestionnaire=true;
        ...
        ${[...new Map(prog.seances.map(s => [s.id,s])).values()].map(seance => `
          <div class="card mb-md">
            <div style="display:flex;align-items:center;
        ...
        <div class="card mb-md"
          ...
          <div style="font-size:.82rem;color:var(--text-secondary);
            ...
            Tu peux le retrouver dans <strong style="color:var(--fd-mint)">
            Programme</strong> et démarrer tes séances directement.
          </div>
          <button onclick="naviguer('training')"
  ...
  // ════════════════════════════════════════════════════════
  // RENDER COACH TAB — Avec bouton Programme IA
  // ════════════════════════════════════════════════════════
    ...
    container.innerHTML = `
      ...
      <div class="card mb-md"
        ...
        <div style="display:flex;align-items:center;gap:14px">
          ...
          <div style="flex:1">
            ...
            <div style="font-size:.72rem;color:var(--text-muted)">
              Questionnaire · Génération automatique
              · Planning adapté
            </div>
          </div>
      ...
      <div class="card mb-md">
        ...
        <div style="margin-top:var(--space-md);padding:var(--space-sm);
                    background:var(--fd-indigo-dim);border-radius:var(--radius-sm)">
  ...
  suggererCharge(exerciceRef) {
    try {
      ...
      return { charge, pourcentage:Math.round(phase.intensite*100), rm1:pr.rm1, phase:phase.nom };
    } catch(e) { return null; }
  },
...
// ✅ Exposer ProgrammeIA globalement
window.Coach        = Coach;
window.ProgrammeIA  = Coach.ProgrammeIA;
console.log('✅ Coach IA v4.0 + Programme IA chargé');
