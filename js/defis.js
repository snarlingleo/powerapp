/* ============================================================
   FitTracker Pro — Défis Hebdomadaires v3.0 CORRIGÉ
...
const Defis = {
  ...
  BANQUE: [
    ...
    { id:'vol_bench_100',  titre:'💪 100 reps Développé couché',
      ...
      type:'reps_cumul', cible:100, xp:300, emoji:'💪', difficulte:2 },
    { id:'vol_squat_80',   titre:'🦵 80 reps de Squat',
      ...
      type:'reps_cumul', cible:80, xp:300, emoji:'🦵', difficulte:2 },
    ...
    { id:'vol_traction_50',titre:'🔗 50 tractions',
      ...
      type:'reps_cumul', cible:50, xp:350, emoji:'🔗', difficulte:3 },
    { id:'vol_curl_120',   titre:'💪 120 reps de Curl',
      ...
      type:'reps_cumul', cible:120, xp:200, emoji:'💪', difficulte:1 },
    ...
    { id:'seance_matin',      titre:'🌅 3 séances avant 10h',
      ...
      cible:3, xp:300, emoji:'🌅', difficulte:3 },
    ...
    { id:'seance_express_3',  titre:'⚡ 3 séances express',
      ...
      cible:3, xp:200, emoji:'⚡', difficulte:2 },
    ...
    { id:'pr_bench',        titre:'🏆 Nouveau PR Développé couché',
      ...
      type:'nouveau_pr', cible:1, xp:500, emoji:'🏆', difficulte:4 },
    ...
    { id:'force_serie_lourde', titre:'💎 Série lourde (5 reps max)',
      ...
      cible:1, xp:200, emoji:'💎', difficulte:3 },
    ...
    { id:'pas_absence', titre:'✅ Zéro absence cette semaine',
      ...
      cible:1, xp:400, emoji:'✅', difficulte:3 },
    ...
    { id:'journal_1',       titre:'📔 1 entrée journal',
      ...
      categorie:'bienetre', type:'journal_semaine',
      cible:1, xp:75, emoji:'📔', difficulte:1 },
    { id:'mesure_semaine',  titre:'⚖️ Prendre ses mesures',
      ...
      cible:1, xp:100, emoji:'⚖️', difficulte:1 },
    ...
    { id:'photo_progression',titre:'📸 Photo de progression',
      ...
      cible:1, xp:150, emoji:'📸', difficulte:1 },
    { id:'humeur_5j',       titre:'😊 Humeur 5 jours',
      ...
      cible:5, xp:100, emoji:'😊', difficulte:1 },
    ...
    { id:'rameur_15min',   titre:'🚣 15 min de rameur',
      ...
      // ✅ FIX — type corrigé en cardio_semaine (cardio_duree non implémentable sans timer)
      type:'cardio_semaine', cible:1,
      xp:150, emoji:'🚣', difficulte:2 },
    ...
    { id:'superset_3', titre:'⚡ 3 supersets cette semaine',
      ...
      cible:3, xp:250, emoji:'⚡', difficulte:2 },
    ...
    { id:'defi_100_series',    titre:'💯 100 séries cette semaine',
      ...
      cible:100, xp:500, emoji:'💯', difficulte:4 },
    { id:'defi_seance_longue', titre:'⏱️ Séance de 90 minutes',
      ...
      cible:90 * 60, xp:300, emoji:'⏱️', difficulte:3 }
  ],
    ...
    const mis_a_jour = defis.map(defi => {
      ...
      try {
        switch(defi.type) {
          ...
          // ✅ FIX — cardio_duree géré (fallback cardio_semaine)
          case 'cardio_duree':
            progression = this._compterCardio(semaine);
            break;
    ...
    try {
      for (let i = 0; i < localStorage.length; i++) {
        ...
        try {
          ...
          // ✅ FIX — vérifier que debut existe avant new Date()
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        try {
          ...
          if ((data?.date||'') >= semaine
              && (data?.seriesCompletes?.length||0) > 0) {
            count++;
          }
        } catch(e) {}
      }
  ...
  // ✅ FIX — Compter aussi les séances full_body comme "express"
    ...
    try {
      for (let i = 0; i < localStorage.length; i++) {
        ...
        try {
          ...
          if (!data?.complete) continue;
          if ((data?.date||'') < semaine) continue;
          // ✅ FIX — inclure full_body et express
  ...
  getStatsDefis() {
    ...
    const completes         = historique.reduce((a,s) => a + s.completes, 0);
    const xpTotal           = historique.reduce((a,s) => a + s.xpGagne, 0);
  ...
  render(container) {
    ...
    let defis = [];
    try {
      defis = this.mettreAJourProgression() || this.genererDefis();
    } catch(e) {
  ...
  // ════════════════════════════════════════════════════════
  // ACTIONS UI — ✅ FIX container resolution
  // ════════════════════════════════════════════════════════
  _getContainer() {
