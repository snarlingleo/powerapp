/* ============================================================
   FitTracker Pro — Social.js v1.0 CORRIGÉ
...
const Social = {
  ...
  async generer(templateId, options = {}) {
    ...
    if (!tmpl) { Utils.toast('Template introuvable', 'error'); return null; }
  ...
  async _storyPR(ctx, tmpl, options) {
    ...
    if (pr) {
      ...
      ctx.fillText(`Equivalent 1RM : ~${pr.rm1}kg`, w/2, 1270);
    }
    ...
    ctx.beginPath(); ctx.moveTo(100, 1340); ctx.lineTo(w-100, 1340); ctx.stroke();

    this._cercle(ctx, w/2, 1470, 90, '#4b4bf9');
    ctx.font = '80px system-ui'; ctx.fillText(profil.avatar||'💪', w/2, 1505);
    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 52px system-ui'; ctx.fillText(profil.nom||'Athlète', w/2, 1590);
    ctx.fillStyle = 'rgba(255,255,255,0.5)';
    ctx.font = '38px system-ui';
    ctx.fillText(`${xp.niveau.emoji} ${xp.niveau.nom}`, w/2, 1650);
    ctx.fillStyle = 'rgba(255,255,255,0.3)';
    ctx.font = '32px system-ui';
    ctx.fillText(this._getHashtags(), w/2, 1760);
  ...
  async _storySemaine(ctx, tmpl, options) {
    ...
    const positions = [
      {x:80, y:500}, {x:560, y:500}, {x:80, y:840}, {x:560, y:840}
    ];

    statsData.forEach((s, i) => {
      const { x, y } = positions[i];
      ctx.fillStyle = 'rgba(255,255,255,0.06)';
      this._roundRect(ctx, x, y, cardW, cardH, 32); ctx.fill();
      ctx.strokeStyle = s.color + '44'; ctx.lineWidth = 2;
      this._roundRect(ctx, x, y, cardW, cardH, 32); ctx.stroke();
      ctx.font = '72px system-ui'; ctx.textAlign = 'center';
      ctx.fillText(s.emoji, x+cardW/2, y+100);
      ctx.fillStyle = s.color; ctx.font = 'bold 80px system-ui';
      ctx.fillText(s.val, x+cardW/2, y+215);
      ctx.fillStyle = 'rgba(255,255,255,0.4)'; ctx.font = '38px system-ui';
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      this._roundRect(ctx, x, y, hmCellW, hmCellH, 14); ctx.fill();

      ctx.fillStyle = etat === 'done' ? '#ffffff' : 'rgba(255,255,255,0.4)';
    ...
    ctx.fillStyle = 'rgba(255,255,255,0.4)'; ctx.font = '42px system-ui';
    ctx.fillText(`${xp.niveau.emoji} ${xp.niveau.nom}`, w/2, 1670);
    ctx.fillStyle = 'rgba(255,255,255,0.25)'; ctx.font = '34px system-ui';
  ...
  async _storyStreak(ctx, tmpl, options) {
    ...
    ctx.font = '220px system-ui'; ctx.textAlign = 'center';
    ctx.fillStyle = 'rgba(255,100,0,0.15)';
    ctx.fillText('🔥', w*0.2, h*0.5);
    ctx.fillText('🔥', w*0.8, h*0.5);

    const bGrad = ctx.createLinearGradient(0, 300, w, 400);
    bGrad.addColorStop(0, '#ff6b00'); bGrad.addColorStop(1, '#ff8d00');
    ctx.fillStyle = bGrad; ctx.fillRect(0, 310, w, 130);
    ctx.fillStyle = '#ffffff'; ctx.font = 'bold 72px system-ui';
    ctx.fillText('🔥 STREAK EN FEU !', w/2, 395);

    ctx.font = '380px system-ui'; ctx.textAlign = 'center';
    ctx.fillText('🔥', w/2, 980);

    ctx.fillStyle = '#ff8d00'; ctx.font = 'bold 260px system-ui';
    ctx.fillText(`${streak.count}`, w/2, 1300);
    ctx.fillStyle = '#ffffff'; ctx.font = 'bold 78px system-ui';
    ctx.fillText('JOURS CONSÉCUTIFS', w/2, 1400);
    ctx.fillStyle = '#f9ef77'; ctx.font = '52px system-ui';
    ctx.fillText(`🏆 Record : ${streak.max} jours`, w/2, 1500);

    this._cercle(ctx, w/2, 1650, 100, '#ff6b00');
    ctx.font = '80px system-ui'; ctx.fillText(profil.avatar||'💪', w/2, 1685);
    ctx.fillStyle = '#ffffff'; ctx.font = 'bold 58px system-ui';
    ctx.fillText(profil.nom||'Athlète', w/2, 1780);
    ctx.fillStyle = 'rgba(255,255,255,0.4)'; ctx.font = '38px system-ui';
    ctx.fillText(`${xp.niveau.emoji} ${xp.niveau.nom}`, w/2, 1840);
    ctx.fillStyle = 'rgba(255,255,255,0.2)'; ctx.font = '32px system-ui';
  ...
  async _storyNiveau(ctx, tmpl, options) {
    ...
    let xp     = { total:0, pourcentage:0,
                   niveau:{ emoji:'💪', numero:1, nom:'Débutant', xpMin:0, xpSuivant:500 } };
    let profil = { nom:'Athlète', avatar:'💪' };
    let streak = { count:0 };
    let total  = 0;
    ...
    nGrad.addColorStop(0, '#4b4bf9'); nGrad.addColorStop(1, '#7b2ff7');
    ctx.fillStyle = nGrad; ctx.fillRect(0, 300, w, 140);
    ctx.fillStyle = '#ffffff'; ctx.font = 'bold 68px system-ui';
    ctx.textAlign = 'center';
    ctx.fillText(`⭐ NIVEAU ${xp.niveau.numero} ATTEINT !`, w/2, 392);

    ctx.font = '380px system-ui'; ctx.fillText(xp.niveau.emoji, w/2, 950);
    ctx.fillStyle = '#f9ef77'; ctx.font = 'bold 120px system-ui';
    ctx.fillText(xp.niveau.nom, w/2, 1100);

    const bX = 100, bY = 1160, bW = w-200, bH = 30;
    ctx.fillStyle = 'rgba(255,255,255,0.1)';
    this._roundRect(ctx, bX, bY, bW, bH, 15); ctx.fill();
    ...
    this._roundRect(ctx, bX, bY, Math.max(30, bW*(xp.pourcentage/100)), bH, 15);
    ctx.fill();

    ctx.fillStyle = 'rgba(255,255,255,0.5)'; ctx.font = '42px system-ui';
    ctx.textAlign = 'center';
    ctx.fillText(`${xp.total.toLocaleString('fr-FR')} XP`, w/2, 1250);
    ...
    [{val:`${total}`,label:'Séances',color:'#8bf0bb'},
     {val:`${streak.count}🔥`,label:'Streak',color:'#f9ef77'}]
      .forEach((s, i) => {
        ...
        ctx.fillStyle = 'rgba(255,255,255,0.06)';
        this._roundRect(ctx, x-200, 1310, 400, 220, 28); ctx.fill();
        ctx.fillStyle = s.color; ctx.font = 'bold 80px system-ui';
        ctx.textAlign = 'center'; ctx.fillText(s.val, x, 1415);
        ctx.fillStyle = 'rgba(255,255,255,0.4)'; ctx.font = '40px system-ui';
        ctx.fillText(s.label, x, 1470);
      });

    this._cercle(ctx, w/2, 1660, 100, '#4b4bf9');
    ctx.font = '80px system-ui'; ctx.textAlign = 'center';
    ...
    ctx.fillStyle = 'rgba(255,255,255,0.25)'; ctx.font = '32px system-ui';
  ...
  // ════════════════════════════════════════════════════════
  // POST CARRÉ — ✅ FIX dépendance circulaire avec Share
  // ════════════════════════════════════════════════════════
  ...
  // ✅ NOUVEAU — Fallback si Share non disponible
    ...
    try {
      ...
      canvas.toBlob(async blob => {
        ...
        switch(reseau) {
          ...
          case 'twitter':   await this._partagerTwitter(file);   break;
  ...
  async telecharger(templateId, options = {}) {
    ...
    try {
      ...
      link.download = `powerapp-${templateId}-${Utils.aujourd_hui()}.png`;
  ...
  async _partagerInstagram(file) {
    ...
    setTimeout(() => URL.revokeObjectURL(link.href), 1000);
    setTimeout(() => {
      Utils.toast('📸 Image téléchargée ! Ouvre Instagram pour la partager.', 'info', 5000);
  ...
  async _partagerWhatsapp(file) {
    if (navigator.share && navigator.canShare?.({ files:[file] })) {
      await navigator.share({ text:`💪 ${this._getHashtags()}`, files:[file] });
    } else {
      const texte = encodeURIComponent(`💪 Mes stats PowerApp — ${this._getHashtags()}`);
  ...
  async _partagerTelegram(file) {
    if (navigator.share && navigator.canShare?.({ files:[file] })) {
      ...
      const txt = encodeURIComponent(`💪 Mes stats PowerApp ${this._getHashtags()}`);
      window.open(`${this.CONFIG.telegramUrl}?url=${url}&text=${txt}`, '_blank');
    }
  },

  async _partagerTwitter(file) {
    const texte = encodeURIComponent(
      `💪 Ma semaine d'entraînement sur PowerApp ! ${this._getHashtags()}`
    );
    window.open(`${this.CONFIG.twitterUrl}?text=${texte}`, '_blank');
  },
            ...
            return `
              <div style="display:flex;justify-content:space-between;
                ...
                <button onclick="Social._celebrerPR('${ref}')"
                        ...
                        style="font-size:.72rem;border-color:var(--fd-lemon);
                               color:var(--fd-lemon)">
                  🎉 Story
                </button>
              </div>`;
      ...
      <div style="display:grid;grid-template-columns:repeat(2,1fr);
        ...
        ${stories.map(tmpl => `
          <div class="card" style="text-align:center;cursor:pointer"
            ...
            <div style="font-weight:700;font-size:.88rem;margin-bottom:4px">
              ${tmpl.nom}
            </div>
            ...
            <div style="font-size:.62rem;color:var(--fd-lavender);font-weight:600">
              ${tmpl.w}×${tmpl.h}px
            </div>
          </div>`).join('')}
      </div>
  ...
  async _ouvrirTemplate(templateId, options = {}) {
    ...
    content.innerHTML = `
      <div style="padding:var(--space-md)">
        <div style="display:flex;justify-content:space-between;
          ...
          <div style="font-weight:700">${tmpl.emoji} ${tmpl.nom}</div>
        ...
        <div id="social-preview-${templateId}"
             style="width:100%;background:var(--bg-input);
                    border-radius:var(--radius-md);min-height:200px;
        ...
        <div style="display:grid;grid-template-columns:repeat(2,1fr);
          ...
          ${[
            { id:'instagram', label:'📸 Instagram', bg:'linear-gradient(45deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888)', color:'white' },
            ...
            <button onclick="Social.partager('${templateId}','${r.id}')"
                    style="padding:var(--space-md);background:${r.bg};
                           color:${r.color};border:none;
    ...
    this.afficherApercu(templateId, `social-preview-${templateId}`, options);
  },
  ...
  _copierHashtags() {
    try {
      navigator.clipboard.writeText(this.CONFIG.hashtags);
      Utils.toast('📋 Hashtags copiés !', 'success');
    } catch(e) {
      Utils.toast('Copie manuelle : ' + this.CONFIG.hashtags, 'info', 5000);
    }
  }
};

window.Social = Social;
console.log('✅ Social.js v1.0 chargé');
