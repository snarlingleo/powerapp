...
const Stats = {
  ...
  getProgressionExercice(ref, periode = 30) {
    ...
    filtre.forEach(h => {
      ...
      if (!parSemaine[sem] || (h.rm1||0) > (parSemaine[sem].rm1||0)) {
        parSemaine[sem] = h;
      }
    });
  ...
  getHeatmap(semaines = 12) { return Tracker.getHeatmapData(semaines * 7); },
  ...
  get1RMTable() {
    ...
    Object.entries(prs).forEach(([ref, pr]) => {
      ...
      if (!ex) return;
      table.push({ ref, nom:ex.nom, emoji:ex.emoji, muscle:ex.muscle,
        rm1:pr.rm1, poids:pr.poids, reps:pr.reps, date:pr.date, ancienPR:pr.ancienPR });
    });
  ...
  getEvolutionPoids() {
    ...
    return Object.entries(parDate)
      .sort(([a],[b]) => a.localeCompare(b))
      .map(([date, poids]) => ({ date, label:Utils.formatDateCourt(date), poids }));
  },
  ...
  getStatsCorps() {
    ...
    const deltaPoids = premiere.poids ? Utils.arrondir(poids - premiere.poids) : null;
    ...
    return { poids, taille, imc, catIMC:catIMCLabel, deltaPoids, calSemaine,
  ...
  render(containerExterne = null, tab = 'dashboard') {
    ...
    // ✅ Détruire TOUS les charts avant re-render
    try { Stats._detruireTousCharts(); } catch(e) {}

    const tabs = ['dashboard','historique','corps','photos','charges','graphiques','calendrier','trophees'];
    const tabLabels = {
      dashboard:'📊 Dashboard', historique:'📋 Historique',
      corps:'⚖️ Corps', photos:'📸 Photos', charges:'🏋️ Charges',
      graphiques:'📈 Graphiques', calendrier:'🗓️ Calendrier', trophees:'🏆 Trophées'
    };
      ...
      <div class="tabs-container">
        ${tabs.map(t => `
          <button class="tab-btn ${tab===t?'active':''}"
                  onclick="Stats.render(document.getElementById('page-stats'),'${t}')">
            ${tabLabels[t]}
          </button>`).join('')}
      </div>
  ...
  renderJournal(container) {
    ...
    container.innerHTML = `
      <div class="card mb-md">
        ...
        <button onclick="Stats._sauvegarderJournal()" class="btn-primary mt-md" style="width:100%">
          💾 Ajouter au journal
        </button>
      </div>
      <div class="section-title">📋 ${journal.length} entrée${journal.length>1?'s':''}</div>
      ${journal.length === 0 ? `
        <div class="card" style="text-align:center;padding:var(--space-xl)">
          <div style="font-size:2.5rem;margin-bottom:var(--space-md)">📔</div>
          <p style="color:var(--text-muted);font-size:.88rem">Ton journal est vide.<br>Commence à écrire !</p>
        </div>` :
        journal.map(e => `
          <div class="card mb-md">
            <div class="flex justify-between items-center mb-sm">
              ...
              <button onclick="Stats._supprimerJournal('${e.id}')"
                      style="background:none;border:none;color:var(--text-muted);font-size:.9rem;cursor:pointer">
                🗑️
              </button>
            </div>
            ${e.humeur ? `<div style="font-size:1.2rem;margin-bottom:4px">${e.humeur}</div>` : ''}
            <p style="font-size:.88rem;color:var(--text-secondary);line-height:1.6;margin:0;white-space:pre-wrap">
              ${e.texte}
            </p>
          </div>`).join('')}
    `;
  ...
  _sauvegarderJournal() {
    ...
    Utils.vibrerSuccess();
    const c = document.getElementById('page-journal') || document.getElementById('stats-content');
    ...
    container.innerHTML = `
      <div class="card mb-md">
        ...
        <div style="margin-top:var(--space-md)">
          ...
          <input class="input mb-md" id="obj-titre" placeholder="ex: Bench press 100kg" />
        ...
        <button onclick="Stats._ajouterObjectif()" class="btn-primary mt-md" style="width:100%">
          ➕ Ajouter
        </button>
      </div>
      <div class="section-title">📋 ${objectifs.length} objectif${objectifs.length>1?'s':''}</div>
      ${objectifs.length === 0 ? `
        <div class="card" style="text-align:center;padding:var(--space-xl)">
          <div style="font-size:2.5rem;margin-bottom:var(--space-md)">🎯</div>
          <p style="color:var(--text-muted);font-size:.88rem">Aucun objectif défini.<br>Crée ton premier objectif !</p>
        </div>` :
        objectifs.map(o => {
          const pct = Tracker.calculerProgressionObjectif(o);
          return `
            <div class="card mb-md" style="${o.complete?'border-color:var(--fd-mint);background:rgba(139,240,187,0.05)':''}">
              <div class="flex justify-between items-center mb-sm">
                <div style="font-weight:700;font-size:.92rem">
                  ${o.complete?'✅ ':'🎯 '}${o.titre||o.description}
                </div>
                ${!o.complete ? `
                  <button onclick="Stats._completerObjectif('${o.id}')" class="btn-secondary btn-sm">✓ Atteint</button>` : `
                  <span class="chip chip-mint" style="font-size:.65rem">Accompli !</span>`}
              </div>
              ${o.valeurActuelle && o.valeurCible ? `
                <div class="flex justify-between" style="font-size:.75rem;margin-bottom:4px;color:var(--text-muted)">
                ...
                <div class="progress-bar">
                  <div class="progress-fill" style="width:${pct}%;background:${o.complete?'var(--fd-mint)':'var(--fd-indigo)'}"></div>
                </div>` : ''}
              <div style="font-size:.65rem;color:var(--text-muted);margin-top:var(--space-xs)">
                Créé le ${Utils.formatDateCourt(o.dateCreation)}
                ${o.dateCompletion ? ` · Atteint le ${Utils.formatDateCourt(o.dateCompletion)}` : ''}
              </div>
            </div>`;
        }).join('')}
    `;
  ...
  _ajouterObjectif() {
    ...
    Tracker.ajouterObjectif({ titre, description:titre,
      valeurActuelle:isNaN(actuel)?null:actuel,
      valeurCible:isNaN(cible)?null:cible, unite });
    Utils.toast('🎯 Objectif ajouté !', 'success');
    const c = document.getElementById('page-objectifs') || document.getElementById('stats-content');
    if (c) this.renderObjectifs(c);
  },

  _completerObjectif(id) {
    Tracker.mettreAJourObjectif(id, { complete:true, dateCompletion:Utils.aujourd_hui() });
  ...
  renderBlessures(container) {
    ...
    const zones = ['Épaule gauche','Épaule droite','Genou gauche','Genou droit',
    ...
    container.innerHTML = `
      <div class="card mb-md">
          ...
          <div style="display:flex;gap:var(--space-sm);margin-bottom:var(--space-md)">
            ${severites.map(s => `
              <button onclick="Stats._selectSeverite(${s.val}, this)"
                      ...
                      style="flex:1;font-size:.78rem;${s.val===1?'border-color:var(--fd-lemon);background:rgba(249,239,119,0.1)':''}">
                ${s.label}
              </button>`).join('')}
          </div>
        ...
        <button onclick="Stats._ajouterBlessure()" class="btn-primary mt-md"
                style="width:100%;background:var(--fd-coral);box-shadow:0 4px 20px rgba(255,141,150,0.3)">
          🩹 Déclarer
        </button>
      </div>
      ${actives.length > 0 ? `
        ...
        ${actives.map(b => `
          <div class="card mb-md" style="border-color:rgba(255,141,150,0.4);background:rgba(255,141,150,0.05)">
            <div class="flex justify-between items-center mb-sm">
              <div>
                <div style="font-weight:700;font-size:.92rem">🩹 ${b.zone}</div>
                <div style="font-size:.72rem;color:var(--text-muted)">
                  ${Utils.formatDateCourt(b.date)} · ${severites.find(s=>s.val===b.severite)?.label||''}
                </div>
              </div>
            ...
            ${b.notes ? `<p style="font-size:.82rem;color:var(--text-secondary);margin:0;line-height:1.5">${b.notes}</p>` : ''}
          </div>`).join('')}` : `
        <div class="card mb-md" style="text-align:center;padding:var(--space-lg);border-color:var(--fd-mint)">
          <div style="font-size:1.5rem">✅</div>
          <div style="font-size:.85rem;color:var(--fd-mint);font-weight:600;margin-top:4px">Aucune blessure active</div>
        </div>`}
      ${gueries.length > 0 ? `
        ...
        ${gueries.map(b => `
          <div class="card mb-md" style="opacity:.6">
            ...
            <div style="font-size:.68rem;color:var(--text-muted)">
              ${Utils.formatDateCourt(b.date)} → ${Utils.formatDateCourt(b.dateGuerison||'')}
            </div>
          </div>`).join('')}` : ''}
    `;
  },
  ...
  _ajouterBlessure() {
    ...
    const c = document.getElementById('page-blessures') || document.getElementById('stats-content');
    ...
    el.innerHTML = `
      ...
      <div class="card mb-md">
        ...
        <div class="flex items-center justify-between mt-md">
          <div style="text-align:center">
            <div style="font-size:1.2rem;font-weight:700;color:var(--fd-indigo)">${Utils.formatVolume(comp.cette)}</div>
            <div style="font-size:.72rem;color:var(--text-muted)">Cette semaine</div>
          </div>
          <div style="text-align:center">
            <div style="font-size:1.6rem;font-weight:800;color:${comp.delta>=0?'var(--fd-mint)':'var(--fd-coral)'}">
              ${comp.delta>=0?'+':''}${comp.delta}%
            </div>
            <div style="font-size:.65rem;color:var(--text-muted)">évolution</div>
          </div>
          <div style="text-align:center">
            <div style="font-size:1.2rem;font-weight:700;color:var(--text-secondary)">${Utils.formatVolume(comp.prec)}</div>
      ...
      <div class="card mb-md">
        ...
        <div class="flex gap-md mt-md">
          ${[
            { label:'Récupération', val:dash.scoreForme.recup,      color:'var(--fd-mint)'     },
            { label:'Assiduité',    val:dash.scoreForme.assiduite,   color:'var(--fd-indigo)'   },
            { label:'Progression',  val:dash.scoreForme.progression, color:'var(--fd-lavender)' }
          ].map(s => `
            <div style="flex:1;text-align:center">
              <div style="font-size:1.1rem;font-weight:800;color:${s.color}">${s.val}%</div>
              <div style="font-size:.65rem;color:var(--text-muted);margin-top:2px">${s.label}</div>
              <div style="height:4px;background:var(--bg-input);border-radius:99px;margin-top:4px;overflow:hidden">
                <div style="height:100%;width:${s.val}%;background:${s.color};border-radius:99px;transition:width 1s"></div>
              </div>
            </div>`).join('')}
        </div>
      </div>
          ...
          top.map((ex, i) => `
            <div class="flex items-center justify-between"
              ...
              <div class="flex items-center gap-md">
                ...
                <div>
                  <div style="font-size:.88rem;font-weight:600">${ex.emoji} ${ex.nom}</div>
                  <div style="font-size:.68rem;color:var(--fd-mint)">${ex.muscle}</div>
                </div>
              </div>
              <div style="text-align:right">
                <div style="font-size:.9rem;font-weight:700;color:var(--fd-indigo)">${ex.poids}kg × ${ex.reps}</div>
                <div style="font-size:.68rem;color:var(--fd-lavender)">1RM: ${ex.rm1}kg</div>
              </div>
            </div>`).join('')}
      </div>
      ...
      <div class="card mb-md">
        ...
        <canvas id="chart-volume" height="160" style="margin-top:var(--space-sm)"></canvas>
      </div>

      ${rpe.length > 1 ? `
        <div class="card mb-md">
          <div class="card-label">😤 RPE moyen par semaine</div>
          <canvas id="chart-rpe" height="140" style="margin-top:var(--space-sm)"></canvas>
        </div>` : ''}
      ...
      if (cvol && vol.length > 0) {
        Stats._charts['chart-volume'] = new Chart(cvol, {
          ...
          data: {
            ...
            datasets: [{ data:vol.map(v=>v.volume), backgroundColor:'rgba(75,75,249,0.7)',
              borderColor:'#4b4bf9', borderWidth:2, borderRadius:6 }]
          },
      ...
      if (crpe && rpe.length > 1) {
        Stats._charts['chart-rpe'] = new Chart(crpe, {
          ...
          data: {
            ...
            datasets: [{ data:rpe.map(r=>r.rpe), borderColor:'#ff8d96',
              backgroundColor:'rgba(255,141,150,0.1)', borderWidth:2,
              pointRadius:4, pointBackgroundColor:'#ff8d96', tension:0.4, fill:true }]
          },
          options: Stats._chartOptions({
            scales: {
              x: { ticks:{color:'#888',font:{size:10}}, grid:{color:'rgba(255,255,255,0.05)'} },
              y: { min:0, max:10, ticks:{color:'#888',font:{size:10},callback:v=>`${v}/10`},
  ...
  _renderRepartitionMuscles() {
    ...
    if (!muscles.length) return `<p style="color:var(--text-muted);text-align:center;padding:var(--space-md);font-size:.85rem">Pas encore de données</p>`;
        ...
        ${muscles.slice(0,6).map((m, i) => {
          ...
          return `
            <div style="margin-bottom:var(--space-sm)">
              <div class="flex justify-between" style="font-size:.78rem;margin-bottom:3px">
                <span style="font-weight:600">${m.muscle}</span>
                <span style="color:var(--text-muted)">${pct}% · ${Utils.formatVolume(m.volume)}</span>
              </div>
              <div style="height:6px;background:var(--bg-input);border-radius:99px;overflow:hidden">
                <div style="height:100%;width:${pct}%;background:${colors[i]};border-radius:99px;transition:width 1s .${i}s"></div>
              </div>
            </div>`;
        }).join('')}
      </div>`;
  ...
  _renderHistorique(el, recherche = '') {
    ...
    const seances = recherche
      ? toutesSeances.filter(s => {
          const nom = (window.SEANCES_BASE?.[s.id]?.nom||s.id||'').toLowerCase();
          return nom.includes(recherche.toLowerCase()) || s.date.includes(recherche);
        })
      : toutesSeances;
    ...
    el.innerHTML = `
      <div class="search-container">
        ...
        <input class="input search-input" id="historique-search"
               placeholder="Rechercher une séance..." value="${recherche}"
               oninput="Stats._filtrerHistorique(this.value)" />
        ${recherche ? `<button class="search-clear" onclick="Stats._filtrerHistorique('')">✕</button>` : ''}
      </div>

      <div class="flex justify-between items-center mb-md">
        <div class="card-label" style="margin:0">
          📋 ${seances.length} séance${seances.length>1?'s':''}
          ${recherche?`<span style="color:var(--fd-indigo)"> · "${recherche}"</span>`:''}
        </div>
        <button onclick="Stats._exportHistorique()"
                style="padding:6px 12px;border-radius:var(--radius-full);
                       border:1px solid var(--border-color);background:var(--bg-input);
                       color:var(--text-primary);font-size:.72rem;font-weight:600;cursor:pointer">
          📤 CSV
        </button>
      </div>
      ...
      ${seances.length === 0 ? `
        <div class="card" style="text-align:center;padding:var(--space-xl)">
          ...
          <p style="color:var(--text-muted)">
            ${recherche ? 'Aucun résultat pour cette recherche.' : 'Aucune séance enregistrée.'}
          </p>
          ${!recherche ? `<button class="btn-primary mt-md" onclick="naviguer('live')">⚡ Démarrer</button>` : ''}
        </div>` :
        seances.map(s => {
          const nom   = window.SEANCES_BASE?.[s.id]?.nom   || s.id || 'Séance';
          const emoji = window.SEANCES_BASE?.[s.id]?.emoji || '💪';
          return `
            <div class="card mb-md" onclick="Stats._afficherDetailSeance('${s.date}','${s.id}')"
                 style="cursor:pointer">
              <div class="flex items-center justify-between mb-sm">
                <div>
                  <div style="font-weight:700;font-size:.95rem">${emoji} ${nom}</div>
                  <div style="font-size:.72rem;color:var(--text-muted)">${Utils.formatDateLong(s.date)}</div>
                </div>
                <div style="text-align:right">
                  <div style="font-size:.82rem;font-weight:700;color:var(--fd-indigo)">${Utils.formatDuree(s.duree||0)}</div>
                  <div style="font-size:.68rem;color:var(--text-muted)">${Utils.formatVolume(s.volumeTotal||0)}</div>
                </div>
              </div>
              <div style="display:flex;flex-wrap:wrap;gap:4px">
                ${(s.exercicesResume||[]).map(e => `
                  <span style="padding:2px 6px;font-size:.65rem;background:var(--bg-input);
                               border:1px solid var(--border-color);border-radius:var(--radius-full)">
                    ${e.emoji} ${e.nom} <span style="color:var(--fd-indigo)">${e.maxPoids}kg</span>
                  </span>`).join('')}
              </div>
              <div class="flex gap-md mt-sm" style="font-size:.72rem;color:var(--text-muted)">
                <span>📊 ${s.series?.length||0} séries</span>
                ${s.rpesMoyen?`<span>😤 RPE ${s.rpesMoyen}</span>`:''}
                ${(s.prs?.length||0)>0?`<span style="color:var(--fd-lemon)">🏆 ${s.prs.length} PR</span>`:''}
              </div>
            </div>`;
        }).join('')}
    `;
  ...
  _afficherDetailSeance(date, seanceId) {
    ...
    content.innerHTML = `
      ...
      <div class="stats-grid mb-md">
        <div class="stat-card"><span class="stat-value" style="font-size:1rem">${Utils.formatDuree(seanceData.duree||0)}</span><span class="stat-label">Durée</span></div>
        <div class="stat-card"><span class="stat-value" style="font-size:1rem">${Utils.formatVolume(seanceData.volumeTotal||0)}</span><span class="stat-label">Volume</span></div>
        <div class="stat-card"><span class="stat-value" style="font-size:1rem">${seanceData.series?.length||0}</span><span class="stat-label">Séries</span></div>
        <div class="stat-card"><span class="stat-value" style="font-size:1rem">${seanceData.rpesMoyen||'—'}</span><span class="stat-label">RPE</span></div>
      </div>
      <div class="card-label mb-sm">Exercices</div>
      ${[...new Set((seanceData.series||[]).map(s=>s.exerciceRef))].map(ref => {
        const ex     = window.EXERCICES?.[ref]||{};
          ...
          <div class="card mb-sm">
            <div class="flex items-center justify-between mb-sm">
              <span style="font-weight:700;font-size:.88rem">
                ${ex.emoji||'💪'} ${ex.nom||ref}
                ${isPR?`<span style="color:var(--fd-lemon);margin-left:4px">🏆 PR</span>`:''}
              </span>
              <span class="chip chip-indigo" style="font-size:.65rem">${ex.muscle||''}</span>
            </div>
            ${series.map((sr,i) => `
              <div class="flex justify-between" style="font-size:.78rem;padding:3px 0;border-bottom:1px solid var(--border-color)">
                <span style="color:var(--text-muted)">Série ${i+1}</span>
                <span style="font-weight:600">${sr.poids}kg × ${sr.reps}
                  ${sr.rpe?`<span style="color:var(--fd-coral);font-size:.7rem"> RPE ${sr.rpe}</span>`:''}
                </span>
              </div>`).join('')}
          </div>`;
      }).join('')}
      ${seanceData.note?`<div class="card mt-md" style="background:rgba(75,75,249,0.08)"><div class="card-label">📔 Note</div><p style="font-size:.85rem;margin-top:var(--space-xs);color:var(--text-secondary)">${seanceData.note}</p></div>`:''}
    `;
  ...
  _exportHistorique() {
    ...
    const csv = ['Date,Séance,Durée(s),Volume(kg),Séries,RPE']
      .concat(seances.map(s => [
        ...
        s.duree||0, Math.round(s.volumeTotal||0), s.series?.length||0, s.rpesMoyen||''
      ].join(','))).join('\n');
    const blob = new Blob([csv], {type:'text/csv;charset=utf-8;'});
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement('a');
    a.href = url; a.download = `powerapp-historique-${Utils.aujourd_hui()}.csv`; a.click();
  ...
  _renderPhotos(el) {
    ...
    const types  = [
      {val:'front',label:'Face',emoji:'⬆️'},{val:'side',label:'Côté',emoji:'➡️'},
      {val:'back',label:'Dos',emoji:'⬇️'},{val:'custom',label:'Autre',emoji:'📷'}
    ];
    el.innerHTML = `
      <div class="card mb-md">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--space-sm);margin-top:var(--space-md)">
          <div>
            ...
            <select class="input" id="photo-type">
              ${types.map(t=>`<option value="${t.val}">${t.emoji} ${t.label}</option>`).join('')}
            </select>
          </div>
          <div>
        ...
        <button class="btn-primary mt-md" onclick="document.getElementById('photo-file').click()">
          📷 Choisir une photo
        </button>
      </div>
      ${photos.length >= 2 ? `
        <div class="card mb-md">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--space-md);margin-top:var(--space-md)">
            <div style="text-align:center">
              <div style="font-size:.72rem;font-weight:700;color:var(--text-muted);margin-bottom:var(--space-sm)">AVANT</div>
              <img src="${photos[photos.length-1].image}" style="width:100%;border-radius:var(--radius-md);aspect-ratio:3/4;object-fit:cover" />
            </div>
            <div style="text-align:center">
              <div style="font-size:.72rem;font-weight:700;color:var(--fd-indigo);margin-bottom:var(--space-sm)">MAINTENANT</div>
              <img src="${photos[0].image}" style="width:100%;border-radius:var(--radius-md);aspect-ratio:3/4;object-fit:cover" />
            </div>
          </div>
        </div>` : ''}
      ...
      ${photos.length === 0 ? `
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:var(--space-sm)">
          ${photos.map(p => `
            <div style="position:relative;border-radius:var(--radius-md);overflow:hidden;aspect-ratio:3/4;cursor:pointer"
                 onclick="Stats._voirPhoto('${p.id}')">
              <img src="${p.image}" style="width:100%;height:100%;object-fit:cover" />
              <div style="position:absolute;bottom:0;left:0;right:0;background:linear-gradient(transparent,rgba(0,0,0,0.7));padding:var(--space-xs);font-size:.6rem;color:white">
  ...
  async _traiterPhoto(input) {
    ...
    } catch(e) { Utils.toast('❌ Erreur lecture photo','error'); }
  },
  ...
  _voirPhoto(id) {
    ...
    if (!modal||!content) return;
    content.innerHTML = `
      <img src="${photo.image}" style="width:100%;border-radius:var(--radius-md);max-height:70vh;object-fit:contain;margin-bottom:var(--space-md)" />
      <div style="font-size:.82rem;color:var(--text-muted);margin-bottom:var(--space-md)">
        📅 ${photo.date} ${photo.poids?`· ⚖️ ${photo.poids}kg`:''} ${photo.note?`<br>📝 ${photo.note}`:''}
      </div>
      <div class="flex gap-md">
        <button class="btn-secondary" style="flex:1" onclick="Stats._telechargerPhoto('${id}')">💾 Télécharger</button>
        <button style="flex:1;padding:var(--space-md) var(--space-lg);background:var(--fd-coral-dim);color:var(--fd-coral);border:1px solid rgba(255,141,150,0.3);border-radius:var(--radius-full);font-size:.9rem;font-weight:600;cursor:pointer"
                onclick="Stats._supprimerPhoto('${id}')">🗑️ Supprimer</button>
      </div>
    `;
    modal.classList.remove('hidden');
    const closeBtn = document.getElementById('modal-info-close');
  ...
  _telechargerPhoto(id) {
    ...
    if (!photo) return;
    const a = document.createElement('a');
    a.href = photo.image; a.download = `powerapp-photo-${photo.date}.jpg`; a.click();
  },
  ...
  _renderCorps(el) {
    ...
    el.innerHTML = `
      <div class="card mb-md" style="background:linear-gradient(135deg,rgba(75,75,249,0.2),rgba(139,240,187,0.1))">
        <div class="card-label">🧮 Bilan corporel</div>
        <div class="stats-grid mt-md">
          <div class="stat-card"><span class="stat-value">${stats.poids||'—'}</span><span class="stat-label">Poids (kg)</span></div>
          <div class="stat-card"><span class="stat-value">${stats.taille||'—'}</span><span class="stat-label">Taille (cm)</span></div>
          <div class="stat-card"><span class="stat-value" style="color:var(--fd-mint)">${stats.imc||'—'}</span><span class="stat-label">IMC</span></div>
          <div class="stat-card"><span class="stat-value" style="color:var(--fd-lemon)">${stats.calSemaine||0}</span><span class="stat-label">Cal/sem</span></div>
        </div>
        ${catIMC?`<div style="margin-top:var(--space-md);padding:var(--space-sm);background:var(--fd-mint-dim);border:1px solid rgba(139,240,187,0.2);border-radius:var(--radius-sm);text-align:center;font-size:.82rem;color:var(--fd-mint);font-weight:600">${catIMC}</div>`:''}
        ${stats.deltaPoids!==null?`<div style="margin-top:var(--space-sm);text-align:center;font-size:.8rem;color:var(--text-muted)">Depuis le début : <span style="font-weight:700;color:${stats.deltaPoids<=0?'var(--fd-mint)':'var(--fd-coral)'}">${stats.deltaPoids>0?'+':''}${stats.deltaPoids} kg</span></div>`:''}
      </div>
      ...
      <div class="card mb-md">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--space-sm);margin-top:var(--space-md)">
          ...
          ].map(f=>`<div><div class="input-label">${f.label}</div><input class="input" id="${f.id}" type="number" placeholder="${f.label}" value="${f.val}" step="0.1" /></div>`).join('')}
        </div>
        <button class="btn-primary mt-md" onclick="Stats._sauvegarderMesure()">💾 Enregistrer</button>
      </div>

      <div class="card mb-md">
        <div class="card-label">⚖️ Évolution du poids</div>
        ${evolPoids.length<2?`<p style="text-align:center;padding:var(--space-xl);color:var(--text-muted);font-size:.85rem">Ajoute au moins 2 mesures</p>`:`<canvas id="chart-poids" height="160" style="margin-top:var(--space-sm)"></canvas>`}
      </div>

      <div class="card mb-md">
        <div class="card-label">📐 Évolution mensurations</div>
        ${evolMens.length<2?`<p style="text-align:center;padding:var(--space-xl);color:var(--text-muted);font-size:.85rem">Ajoute au moins 2 mesures</p>`:`<canvas id="chart-mensur" height="160" style="margin-top:var(--space-sm)"></canvas>`}
      </div>

      <div class="card">
        <div class="card-label">📋 Historique</div>
        ${Tracker.getMesures().length===0?`<p style="color:var(--text-muted);text-align:center;padding:var(--space-lg);font-size:.85rem">Aucune mesure</p>`:''}
        ${[...Tracker.getMesures()].reverse().slice(0,10).map(m=>`
          <div style="padding:var(--space-sm) 0;border-bottom:1px solid var(--border-color);font-size:.82rem">
            <div style="font-weight:600;color:var(--fd-indigo);margin-bottom:2px">${Utils.formatDateCourt(m.date)}</div>
      ...
      if (evolPoids.length >= 2) {
        ...
        if (c) Stats._charts['chart-poids'] = new Chart(c, {
          ...
          data: {
            ...
            datasets: [{ data:evolPoids.map(p=>p.poids), borderColor:'#ff8d96',
              backgroundColor:'rgba(255,141,150,0.1)', borderWidth:2,
              pointRadius:4, pointBackgroundColor:'#ff8d96', tension:0.4, fill:true }]
          },
          options: Stats._chartOptions()
      ...
      if (evolMens.length >= 2) {
        ...
        if (c) {
          ...
          keys.forEach((key,i) => {
            ...
            if (vals.some(v=>v>0)) {
              datasets.push({ data:vals, borderColor:colors[i],
                backgroundColor:colors[i]+'22', borderWidth:2,
                pointRadius:3, pointBackgroundColor:colors[i],
    ...
    el.innerHTML = `
      <div class="card mb-md">
        ...
        <select id="select-exercice" class="input mt-md">
          ...
          ${exRefs.map(ref=>`<option value="${ref}">${window.EXERCICES[ref]?.emoji||''} ${window.EXERCICES[ref]?.nom||ref}</option>`).join('')}
        </select>
      </div>
      <div id="detail-exercice"></div>
      <div class="card">
        <div class="card-label">📊 Tableau 1RM complet</div>
        ${table.length===0?`<p style="color:var(--text-muted);text-align:center;padding:var(--space-lg)">Lance tes séances pour voir tes 1RM !</p>`:''}
        ${table.map((ex,i) => `
          <div class="flex items-center justify-between" style="padding:var(--space-sm) 0;border-bottom:1px solid var(--border-color)">
            ...
            <div style="text-align:right">
              <div style="font-size:.9rem;font-weight:700;color:var(--fd-indigo)">1RM: ${ex.rm1}kg</div>
              <div style="font-size:.68rem;color:var(--text-muted)">
                ${ex.poids}kg × ${ex.reps}
                ${ex.ancienPR?.rm1?`<span style="color:var(--fd-mint)"> ↑${ex.rm1-ex.ancienPR.rm1}kg</span>`:''}
              </div>
            </div>
          </div>`).join('')}
      </div>
    ...
    el.innerHTML = `
      <div class="card mb-md">
        <div class="flex items-center justify-between mb-md">
          <div class="card-label" style="margin:0">${ex?.emoji||'💪'} ${ex?.nom||ref}</div>
          <button onclick="Stats._ajouterNoteExo('${ref}')"
                  style="padding:4px 12px;border-radius:var(--radius-full);border:1px solid var(--border-color);background:var(--bg-input);color:var(--text-primary);font-size:.72rem;font-weight:600;cursor:pointer">
            📝 Note
          </button>
        </div>
        <div class="stats-grid mb-md">
          <div class="stat-card"><span class="stat-value" style="color:var(--fd-lemon)">${pr?.rm1||0}kg</span><span class="stat-label">1RM Estimé</span></div>
          <div class="stat-card"><span class="stat-value" style="color:var(--fd-indigo)">${pr?.poids||0}kg</span><span class="stat-label">Meilleur</span></div>
          <div class="stat-card"><span class="stat-value" style="color:var(--fd-mint)">${pr?.reps||0}</span><span class="stat-label">Meilleur reps</span></div>
          <div class="stat-card"><span class="stat-value" style="font-size:.9rem">${pr?.date?Utils.formatDateCourt(pr.date):'—'}</span><span class="stat-label">Date PR</span></div>
        </div>
        ${pr?.rm1?`
          <div class="card-label mb-sm">📊 Zones d'entraînement</div>
          <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:4px;margin-bottom:var(--space-md)">
            ${[{pct:50,label:'Endurance',color:'#8bf0bb'},{pct:65,label:'Volume',color:'#4b4bf9'},
               {pct:75,label:'Hypertro.',color:'#bfa1ff'},{pct:85,label:'Force',color:'#f9ef77'},
               {pct:95,label:'Max',color:'#ff8d96'}].map(z=>`
              <div style="text-align:center;padding:var(--space-xs);background:${z.color}22;border:1px solid ${z.color}44;border-radius:var(--radius-sm)">
                <div style="font-size:.75rem;font-weight:700;color:${z.color}">${Math.round(pr.rm1*z.pct/100)}kg</div>
                <div style="font-size:.58rem;color:var(--text-muted)">${z.pct}%</div>
                <div style="font-size:.58rem;color:var(--text-muted)">${z.label}</div>
              </div>`).join('')}
          </div>`:''}
        ${prog.length>1?`<div class="card-label" style="font-size:.72rem">📈 Progression 1RM (60 jours)</div><canvas id="chart-exo" height="140" style="margin-top:var(--space-sm)"></canvas>`:`<p style="color:var(--text-muted);text-align:center;font-size:.85rem;padding:var(--space-md)">Pas encore assez de données</p>`}
        ${notes.length>0?`<div class="card-label mt-md mb-sm">📝 Mes notes</div>${notes.slice(0,3).map(n=>`<div style="font-size:.78rem;padding:var(--space-xs) 0;border-bottom:1px solid var(--border-color);color:var(--text-secondary)"><span style="color:var(--fd-indigo);font-size:.65rem">${n.date}</span><br>${n.note}</div>`).join('')}`:''}
      </div>
    `;
    ...
    if (prog.length>1) {
      requestAnimationFrame(() => {
        ...
        if (canvas) Stats._charts['chart-exo'] = new Chart(canvas, {
          type: 'line',
          data: { labels:prog.map(p=>p.label), datasets:[{ data:prog.map(p=>p.rm1),
            borderColor:'#bfa1ff', backgroundColor:'rgba(191,161,255,0.1)',
            borderWidth:2, pointRadius:4, pointBackgroundColor:'#bfa1ff', tension:0.4, fill:true }]},
          options: Stats._chartOptions()
        });
      });
    }
  ...
  _ajouterNoteExo(ref) {
    ...
    if (!modal||!content) return;
    content.innerHTML = `
      <h3 style="margin-bottom:var(--space-md)">📝 Note — ${ex?.emoji||''} ${ex?.nom||ref}</h3>
      ...
      <button class="btn-primary mt-md" onclick="Stats._sauvegarderNoteExo('${ref}')">💾 Sauvegarder</button>
    `;
    modal.classList.remove('hidden');
    const closeBtn = document.getElementById('modal-info-close');
    if (closeBtn) closeBtn.onclick = ()=>modal.classList.add('hidden');
  ...
  // ════════════════════════════════════════════════════════
  // GRAPHIQUES TAB — ✅ Fix charts dupliqués
  // ════════════════════════════════════════════════════════
      ...
      <div class="card mb-md">
        ...
        <div style="display:flex;gap:var(--space-sm);flex-wrap:wrap;margin-bottom:var(--space-sm)">
          <select id="sel-graph-exo" class="input" style="flex:2;min-width:140px">
            <option value="">-- Choisir un exercice --</option>
            ${refs.map(r=>`<option value="${r}">${window.EXERCICES[r]?.emoji||''} ${window.EXERCICES[r]?.nom||r}</option>`).join('')}
          </select>
        ...
        <div style="display:flex;gap:6px;margin-bottom:var(--space-md)">
          ${periodes.map((p,i)=>`
            <button data-periode="${p}" onclick="Stats._chartChangerPeriode(this)"
                    style="flex:1;padding:6px 4px;border-radius:var(--radius-full);font-size:.75rem;font-weight:600;cursor:pointer;
        ...
        <div id="chart-prog-placeholder" style="text-align:center;padding:var(--space-xl);color:var(--text-muted);font-size:.85rem">
          Sélectionne un exercice pour voir sa progression
        </div>
        <canvas id="chart-progression" height="200" style="display:none"></canvas>
      </div>
      <div class="card mb-md"><div class="card-label">📊 Volume total — 8 semaines</div><canvas id="chart-vol-8w" height="180" style="margin-top:var(--space-sm)"></canvas></div>
      <div class="card mb-md"><div class="card-label">💪 Top 1RM estimés</div><canvas id="chart-top-rm" height="200" style="margin-top:var(--space-sm)"></canvas></div>
      <div class="card mb-md"><div class="card-label">📅 Séances par jour de la semaine</div><canvas id="chart-jours-sem" height="160" style="margin-top:var(--space-sm)"></canvas></div>
      <div class="card mb-md"><div class="card-label">😤 Évolution RPE moyen</div><canvas id="chart-rpe-evo" height="160" style="margin-top:var(--space-sm)"></canvas></div>
    `;

    // ✅ Détruire avant de créer
    Stats._detruireCharts(['chart-vol-8w','chart-top-rm','chart-jours-sem','chart-rpe-evo']);
    ...
    requestAnimationFrame(() => {
      ...
      if (cvol && vol.length>0) {
        Stats._charts['chart-vol-8w'] = new Chart(cvol, {
          type:'bar',
          data:{ labels:vol.map(v=>v.label), datasets:[{data:vol.map(v=>v.volume),
            backgroundColor:'rgba(75,75,249,0.7)',borderColor:'#4b4bf9',borderWidth:2,borderRadius:6}]},
          options:Stats._chartOptions()
        });
      }
      ...
      if (crm && top.length>0) {
        const colors = ['#4b4bf9','#8bf0bb','#f9ef77','#bfa1ff','#ff8d96','#4b4bf9','#8bf0bb','#f9ef77'];
        Stats._charts['chart-top-rm'] = new Chart(crm, {
          type:'bar',
          data:{ labels:top.map(e=>e.nom), datasets:[{data:top.map(e=>e.rm1),
      ...
      if (cjours) {
        Stats._charts['chart-jours-sem'] = new Chart(cjours, {
          ...
          data:{ labels:['Lun','Mar','Mer','Jeu','Ven','Sam','Dim'],
            datasets:[{data:jours,backgroundColor:'rgba(139,240,187,0.6)',borderColor:'#8bf0bb',borderWidth:2,borderRadius:6}]},
          options:Stats._chartOptions()
        });
      }
      ...
      if (crpe && rpe.length>1) {
        Stats._charts['chart-rpe-evo'] = new Chart(crpe, {
          type:'line',
          data:{ labels:rpe.map(r=>r.semaine), datasets:[{data:rpe.map(r=>r.rpe),
            borderColor:'#ff8d96',backgroundColor:'rgba(255,141,150,0.1)',borderWidth:2,
            pointRadius:4,pointBackgroundColor:'#ff8d96',tension:0.4,fill:true}]},
          options:Stats._chartOptions({
            scales:{ x:{ticks:{color:'#888',font:{size:10}},grid:{color:'rgba(255,255,255,0.05)'}},
                     y:{min:0,max:10,ticks:{color:'#888',font:{size:10},callback:v=>`${v}/10`},grid:{color:'rgba(255,255,255,0.05)'}}}
          })
        });
      }
    });

    document.getElementById('sel-graph-exo')?.addEventListener('change',()=>Stats._chartMettreAJourProg());
    document.getElementById('sel-graph-metric')?.addEventListener('change',()=>Stats._chartMettreAJourProg());
  },

  _chartChangerPeriode(btn) {
    document.querySelectorAll('[data-periode]').forEach(b => {
      b.style.background='var(--bg-input)'; b.style.borderColor='var(--border-color)'; b.style.color='var(--text-muted)';
    });
    btn.style.background='var(--fd-indigo)'; btn.style.borderColor='var(--fd-indigo)'; btn.style.color='white';
  ...
  _chartMettreAJourProg() {
    ...
    const periode = document.querySelector('[data-periode][style*="var(--fd-indigo)"]')?.dataset?.periode||'30';
    ...
    if (!prog.length) {
      if (ph) { ph.style.display='block'; ph.textContent='Pas encore assez de données'; }
      if (canvas) canvas.style.display='none';
      return;
    }
    ...
    const metricLabels = { rm1:'1RM estimé (kg)', poids:'Poids soulevé (kg)', reps:'Répétitions' };
    const color = metricColors[metric];

    Stats._charts['chart-progression'] = new Chart(canvas, {
      type:'line',
      data:{ labels:prog.map(p=>p.label), datasets:[{
        label:metricLabels[metric], data:prog.map(p=>p[metric]||0),
        borderColor:color, backgroundColor:color+'22', borderWidth:2.5,
        pointRadius:5, pointHoverRadius:7, pointBackgroundColor:color, tension:0.35, fill:true
      }]},
      options:Stats._chartOptions({
        plugins:{ legend:{display:true,labels:{color:'#888',font:{size:11}}},
                  tooltip:{backgroundColor:'#09092d',titleColor:'#fff',bodyColor:color,borderColor:color,borderWidth:1}}
      })
    });
  },
  ...
  _renderCalendrier(el, annee=null, mois=null) {
    if (!el) { el=document.getElementById('stats-content'); if (!el) return; }
    ...
    el.innerHTML = `
      <div class="flex items-center justify-between mb-md">
        <button onclick="Stats._renderCalendrier(document.getElementById('stats-content'),${mo===0?an-1:an},${mo===0?11:mo-1})"
                style="width:36px;height:36px;border-radius:50%;border:1px solid var(--border-color);background:var(--bg-input);color:var(--text-primary);font-size:.9rem;cursor:pointer">◄</button>
        <div style="text-align:center">
          <div style="font-weight:700;font-size:1.1rem">${nomsMois[mo]} ${an}</div>
          <div style="font-size:.72rem;color:var(--text-muted)">${seancesMois} séance${seancesMois>1?'s':''} · ${manqueesMois} manquée${manqueesMois>1?'s':''}</div>
        </div>
        <button onclick="${isCurrentMonth?'void 0':`Stats._renderCalendrier(document.getElementById('stats-content'),${mo===11?an+1:an},${mo===11?0:mo+1})`}"
                ${isCurrentMonth?'disabled':''}
                style="width:36px;height:36px;border-radius:50%;border:1px solid var(--border-color);background:var(--bg-input);color:var(--text-primary);font-size:.9rem;cursor:pointer;opacity:${isCurrentMonth?'.3':'1'}">►</button>
      </div>
      <div class="stats-grid mb-md">
        <div class="stat-card"><span class="stat-value" style="color:var(--fd-indigo)">${seancesMois}</span><span class="stat-label">Séances</span></div>
        <div class="stat-card"><span class="stat-value" style="color:var(--fd-coral)">${manqueesMois}</span><span class="stat-label">Manquées</span></div>
        <div class="stat-card"><span class="stat-value" style="color:var(--fd-mint)">${Math.round((seancesMois/Math.max(1,seancesMois+manqueesMois))*100)}%</span><span class="stat-label">Assiduité</span></div>
        <div class="stat-card"><span class="stat-value" style="color:var(--fd-lemon)">${Tracker.getStreak().count}🔥</span><span class="stat-label">Streak</span></div>
      </div>
      <div class="card mb-md" style="padding:var(--space-sm)">
        <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:3px;margin-bottom:4px">
          ${nomsJours.map(j=>`<div style="text-align:center;font-size:.65rem;font-weight:600;color:var(--text-muted);padding:4px 0">${j}</div>`).join('')}
        </div>
        <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:3px">${cellules}</div>
      </div>
  ...
  _afficherJour(dateStr) {
    ...
    el.innerHTML = `
      <div class="card mt-md" style="border-color:${etat==='done'?'var(--fd-indigo)':etat==='missed'?'var(--fd-coral)':etat==='rest'?'var(--fd-mint)':'var(--border-color)'}">
        <div class="flex items-center justify-between mb-md">
          <div>
            <div style="font-weight:700">${Utils.formatDateLong(dateStr)}${dateStr===Utils.aujourd_hui()?'<span class="chip chip-lemon" style="font-size:.6rem;margin-left:4px">Aujourd\'hui</span>':''}</div>
            <div style="font-size:.75rem;color:var(--text-muted)">${etat==='done'?'✅ Séance complétée':etat==='missed'?'❌ Séance manquée':etat==='rest'?'😴 Repos':'⬜ Pas de données'}</div>
          </div>
        ...
        ${seanceData?.complete?`
          ...
          ${[...new Set((seanceData.series||[]).map(s=>s.exerciceRef))].map(ref=>{
            ...
            return `<div class="flex justify-between" style="font-size:.82rem;padding:var(--space-xs) 0;border-bottom:1px solid var(--border-color)"><span>${ex.emoji||'💪'} ${ex.nom||ref}</span><span style="font-weight:600;color:var(--fd-indigo)">${max}kg × ${series.length} séries</span></div>`;
          }).join('')}`:
          `<div style="text-align:center;padding:var(--space-md);color:var(--text-muted)"><div style="font-size:2rem">${etat==='rest'?'😴':'⬜'}</div><div style="font-size:.85rem;margin-top:4px">${etat==='rest'?'Jour de repos':'Aucune donnée'}</div></div>`}
      </div>
    `;
