/* ============================================================
   PowerApp — Nutrition.js v1.1 FINAL
   Suivi macros + Base aliments + Calcul automatique
   + Fix charts dupliqués + Debounce + Rappel eau
   ============================================================ */
...
const Nutrition = {
  ...
  sauvegarderJour(data) {
    Utils.storage.set(`ft_nutrition_${Utils.aujourd_hui()}`, data);
  },
  ...
  getTotauxJour() {
    return this.getJourActuel().repas.reduce(
      (acc, r) => ({
        cal:  acc.cal  + (r.cal  || 0),
  ...
  getHistoriqueNutrition(jours = 7) {
    ...
    for (let i = 0; i < jours; i++) {
      ...
      if (data) {
        ...
        hist.push({ date, label:Utils.formatDateCourt(date), ...totaux, eau:data.eau||0 });
      }
    }
    return hist.reverse();
  },
  ...
  // ✅ Debounce — évite les re-renders multiples
  _renderTimeout: null,

  renderDebounced(container, delai = 150) {
    clearTimeout(this._renderTimeout);
    this._renderTimeout = setTimeout(() => this.render(container), delai);
  },
  ...
  render(container) {
    ...
    const pctEau  = Math.min(100, Math.round(((jour.eau||0) / 1000 / besoins.eau) * 100));
    ...
    container.innerHTML = `
      ...
      <div class="card mb-md"
           style="background:linear-gradient(135deg,rgba(75,75,249,0.15),rgba(139,240,187,0.05));border-color:var(--fd-indigo)">
        <div class="flex justify-between items-center mb-md">
          <div class="card-label" style="margin:0">📊 Bilan du jour</div>
        ...
        <div style="text-align:center;margin-bottom:var(--space-md)">
          <div style="font-size:2.5rem;font-weight:800;color:var(--fd-indigo);line-height:1">
            ${totaux.cal}
          </div>
          <div style="font-size:.75rem;color:var(--text-muted)">/ ${besoins.calories} kcal</div>
          <div style="height:8px;background:var(--bg-input);border-radius:99px;margin-top:8px;overflow:hidden">
            <div style="height:100%;width:${pctCal}%;
                        background:${pctCal>110?'var(--fd-coral)':pctCal>90?'var(--fd-mint)':'var(--fd-indigo)'};
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:var(--space-sm)">
          ${[
            { label:'Protéines', val:totaux.prot, obj:besoins.proteines, pct:pctProt, color:'var(--fd-coral)',    unite:'g' },
            { label:'Glucides',  val:totaux.gluc, obj:besoins.glucides,  pct:pctGluc, color:'var(--fd-lemon)',    unite:'g' },
            { label:'Lipides',   val:totaux.lip,  obj:besoins.lipides,   pct:pctLip,  color:'var(--fd-lavender)', unite:'g' }
          ].map(m => `
            <div style="text-align:center;padding:var(--space-sm);background:var(--bg-input);border-radius:var(--radius-sm)">
              <div style="font-size:.9rem;font-weight:800;color:${m.color}">${m.val}${m.unite}</div>
              <div style="font-size:.6rem;color:var(--text-muted)">/ ${m.obj}${m.unite}</div>
              <div style="height:4px;background:var(--bg-card);border-radius:99px;margin-top:4px;overflow:hidden">
                <div style="height:100%;width:${m.pct}%;background:${m.color};border-radius:99px"></div>
              </div>
              <div style="font-size:.6rem;color:${m.color};margin-top:2px;font-weight:600">${m.pct}%</div>
            </div>`).join('')}
        </div>
      </div>
      ...
      <div class="card mb-md">
        <div class="flex justify-between items-center mb-sm">
          ...
          <div style="font-size:.82rem;font-weight:700;color:var(--fd-indigo)">
            ${((jour.eau||0)/1000).toFixed(1)}L / ${besoins.eau}L
          </div>
        </div>
        <div style="height:8px;background:var(--bg-input);border-radius:99px;overflow:hidden;margin-bottom:var(--space-sm)">
          <div style="height:100%;width:${pctEau}%;background:var(--fd-indigo);border-radius:99px;transition:width .5s"></div>
        </div>
        <div style="display:flex;gap:var(--space-sm)">
          ${[150,250,330,500].map(ml => `
            <button onclick="Nutrition._ajouterEauUI(${ml})"
                    style="flex:1;padding:8px 4px;font-size:.72rem;font-weight:600;
                           background:rgba(75,75,249,0.1);border:1px solid rgba(75,75,249,0.25);
                           border-radius:var(--radius-full);color:var(--fd-indigo);cursor:pointer">
              +${ml}ml
            </button>`).join('')}
        </div>
      </div>
      ...
      ${(jour.eau||0) < (besoins.eau * 1000 * 0.5) ? `
        <div class="eau-rappel-banner mb-md">
          ...
          <span class="eau-rappel-text">
            Tu n'as bu que ${((jour.eau||0)/1000).toFixed(1)}L aujourd'hui. Objectif : ${besoins.eau}L
          </span>
          <button class="eau-rappel-btn" onclick="Nutrition._ajouterEauUI(250)">+250ml</button>
        </div>` : ''}
      ...
      <div class="card mb-md">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--space-sm);margin-bottom:var(--space-sm)">
          <div>
            ...
            <select class="input" id="nutri-repas">
              ${repasTypes.map(r=>`<option value="${r.val}">${r.label}</option>`).join('')}
            </select>
          </div>
          <div>
            <div class="input-label">Quantité (g/ml)</div>
            <input class="input" id="nutri-qte" type="number" placeholder="100" value="100" min="1" max="2000" />
          </div>
        </div>
        ...
        <div id="nutri-aliments-list"
             style="max-height:200px;overflow-y:auto;border:1px solid var(--border-color);border-radius:var(--radius-md);margin-bottom:var(--space-sm)">
          ${this._renderListeAliments('')}
        </div>
        <div id="nutri-selection"
             style="display:none;padding:var(--space-sm);background:rgba(75,75,249,0.1);border-radius:var(--radius-sm);margin-bottom:var(--space-sm);font-size:.82rem">
        </div>
        <button onclick="Nutrition._confirmerAjout()" class="btn-primary" style="width:100%">
          ➕ Ajouter
        </button>
      </div>
      ...
      ${repasTypes.map(type => {
        ...
        const totalRepas = items.reduce((acc,r) => ({cal:acc.cal+r.cal}), {cal:0});
        return `
          <div class="card mb-md">
            <div class="flex justify-between items-center mb-sm">
              <div class="card-label" style="margin:0">${type.label}</div>
              <div style="font-size:.72rem;color:var(--fd-lemon);font-weight:600">${totalRepas.cal} kcal</div>
            </div>
            ${items.map(item => `
              <div style="display:flex;align-items:center;justify-content:space-between;padding:var(--space-xs) 0;border-bottom:1px solid var(--border-color)">
                <div style="display:flex;align-items:center;gap:8px">
                  <span style="font-size:1.1rem">${item.emoji}</span>
                  <div>
                    <div style="font-size:.82rem;font-weight:600">${item.nom}</div>
                    <div style="font-size:.65rem;color:var(--text-muted)">${item.quantite}g · ${item.heure}</div>
                  </div>
                </div>
                <div style="display:flex;align-items:center;gap:8px">
                  <div style="text-align:right">
                    <div style="font-size:.78rem;font-weight:700;color:var(--fd-lemon)">${item.cal} kcal</div>
                    <div style="font-size:.62rem;color:var(--text-muted)">P:${item.prot} G:${item.gluc} L:${item.lip}</div>
                  </div>
                  <button onclick="Nutrition._supprimerAlimentUI(${item.id})"
                          style="background:none;border:none;color:var(--text-muted);font-size:.9rem;cursor:pointer;padding:4px">
                    🗑️
                  </button>
                </div>
              </div>`).join('')}
      ...
      ${hist.length > 1 ? `
        <div class="card mb-md">
          <div class="card-label mb-sm">📈 Calories — 7 derniers jours</div>
          <canvas id="chart-nutri-cal" height="160"></canvas>
        </div>
        <div class="card mb-md">
          <div class="card-label mb-sm">💪 Protéines — 7 derniers jours</div>
          <canvas id="chart-nutri-prot" height="140"></canvas>
        </div>` : ''}

      <!-- ═══ OBJECTIFS MACROS ═══ -->
      <div class="card mb-md" style="background:rgba(75,75,249,0.05);border-color:rgba(75,75,249,0.2)">
        ...
        ${[
          ...
          <div class="flex justify-between items-center"
               style="padding:var(--space-xs) 0;border-bottom:1px solid var(--border-color)">
            <span style="font-size:.82rem">${o.label}</span>
            <span style="font-weight:700;font-size:.88rem;color:${o.color}">${o.val} ${o.unite}</span>
          </div>`).join('')}
        <div style="font-size:.65rem;color:var(--text-muted);margin-top:var(--space-sm);text-align:center">
  ...
  _renderListeAliments(filtre) {
    const aliments = Object.entries(this.ALIMENTS)
      .filter(([,a]) => !filtre || a.nom.toLowerCase().includes(filtre.toLowerCase()));

    if (!aliments.length) {
      return `<div style="padding:var(--space-md);text-align:center;color:var(--text-muted);font-size:.82rem">Aucun aliment trouvé</div>`;
    }

    return aliments.map(([ref, a]) => `
      <div onclick="Nutrition._selectionnerAliment('${ref}')"
           style="display:flex;align-items:center;gap:8px;padding:var(--space-sm) var(--space-md);cursor:pointer;border-bottom:1px solid var(--border-color);transition:background .15s"
        ...
        <div style="flex:1">
          ...
          <div style="font-size:.62rem;color:var(--text-muted)">
            ${a.cal}kcal · P:${a.prot}g G:${a.gluc}g L:${a.lip}g pour 100${a.unite}
          </div>
        </div>
      </div>`).join('');
  },
        ...
        <div style="display:flex;align-items:center;gap:8px">
          ...
          <div>
            ...
            <div style="font-size:.7rem;color:var(--text-muted)">
              ${qte}g → <strong style="color:var(--fd-lemon)">${macros.cal} kcal</strong>
  ...
  // ✅ Une seule version de chaque méthode — avec debounce
  _confirmerAjout() {
  ...
  _ajouterEauUI(ml) {
    ...
    Utils.toast(`💧 +${ml}ml · Total: ${(total/1000).toFixed(1)}L`, 'success', 1500);
    const container = document.getElementById('page-nutrition')
      || document.getElementById('stats-content');
    if (container) this.renderDebounced(container);
  }
...
console.log('✅ Nutrition.js v1.1 chargé');
