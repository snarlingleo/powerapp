/* ============================================================
   PowerApp — Circuit.js v1.0
   Mode Circuit + HIIT + Tabata
   ============================================================ */
...
const Circuit = {
  ...
  MODES: {
    circuit: {
      id:'circuit', nom:'Circuit Training',
      emoji:'🔄', description:'Enchaîne les exercices sans repos entre eux',
      color:'var(--fd-indigo)'
    },
    hiit: {
      id:'hiit', nom:'HIIT',
      emoji:'🔥', description:'Intervalles haute intensité / récupération',
      color:'var(--fd-coral)'
    },
    tabata: {
      id:'tabata', nom:'Tabata',
      emoji:'⚡', description:'20s effort / 10s repos × 8 rounds',
      color:'var(--fd-lemon)'
    },
    amrap: {
      id:'amrap', nom:'AMRAP',
      emoji:'♾️', description:'As Many Rounds As Possible',
      color:'var(--fd-mint)'
    },
    emom: {
      id:'emom', nom:'EMOM',
      emoji:'⏰', description:'Every Minute On the Minute',
  ...
  render(container) {
    ...
    container.innerHTML = `
      ...
      <!-- ═══ CONFIGURATEUR (caché initialement) ═══ -->
      <div id="circuit-config" style="display:none"></div>

      <!-- ═══ PLAYER (caché initialement) ═══ -->
  ...
  _selectionnerMode(modeId) {
    ...
    document.getElementById('circuit-selection').style.display = 'none';
    const config = document.getElementById('circuit-config');
    config.style.display = 'block';

    // Configs par défaut selon mode
    const defaults = {
        ...
        ${modeId !== 'amrap' ? `
          <div style="display:grid;grid-template-columns:1fr 1fr;
            ...
            <div>
              ...
              <input class="input" id="cfg-travail"
                     type="number" value="${def.travail}"
                     min="5" max="300" />
            </div>
            <div>
              <div class="input-label">😴 Repos (sec)</div>
              <input class="input" id="cfg-repos"
                     type="number" value="${def.repos}"
                     min="0" max="300" />
            </div>
            <div>
              <div class="input-label">🔄 Rounds</div>
              <input class="input" id="cfg-rounds"
                     type="number" value="${def.rounds}"
                     min="1" max="20" />
            </div>
            ${modeId !== 'tabata' ? `
              <div>
                ...
                <input class="input" id="cfg-repos-circuit"
                       type="number"
                       value="${def.repos_circuit}"
                       min="0" max="300" />
              </div>` : '<div></div>'}
          </div>` : `
          <div>
            <div class="input-label">⏱️ Durée totale (min)</div>
            <input class="input" id="cfg-temps-total"
                   type="number"
    ...
    // Ajouter 3 exercices par défaut
    this._exercicesConfig = [];
    const defExos = ['squat','pompes','planche'];
    defExos.forEach(ref => this._ajouterExerciceRef(ref));
  },
  ...
  _renderExercicesConfig(exercices) {
    if (!exercices.length) {
      return `<div style="text-align:center;padding:var(--space-md);
    ...
    modal.innerHTML = `
      <div style="width:100%;max-width:480px;margin:0 auto;
        ...
        <div style="overflow-y:auto;flex:1">
          ${exos.map(([ref, ex]) => `
            <div onclick="Circuit._ajouterExerciceRef('${ref}');
  ...
  _lancerConfig(modeId) {
    ...
    const config = {
      ...
      reposCircuit: parseInt(
        document.getElementById('cfg-repos-circuit')?.value
      ) || 60,
      tempsTotalAMRAP: (parseInt(
        document.getElementById('cfg-temps-total')?.value
      ) || 10) * 60
    };
  ...
  _retourSelection() {
    document.getElementById('circuit-selection').style.display = 'block';
    document.getElementById('circuit-config').style.display   = 'none';
    document.getElementById('circuit-player').style.display   = 'none';
    this._arreterPlayer();
  },

  // ════════════════════════════════════════════════════════
  // PLAYER — Cœur du circuit
  // ════════════════════════════════════════════════════════
  _timer:        null,
  _tempsRestant: 0,
  _phase:        'travail', // 'travail' | 'repos' | 'repos_circuit'
  _exoActuel:    0,
  _roundActuel:  1,
  _config:       null,
  _enPause:      false,
  ...
  _renderPlayer() {
    ...
    const exoSuiv    = exoSuivant
      ? (window.EXERCICES||{})[exoSuivant.ref] || {}
      : null;
    ...
    player.innerHTML = `
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;
        ...
        <button id="btn-pause-circuit"
                onclick="Circuit._togglePause()"
                class="btn-secondary"
  ...
  _lancerTimer() {
    ...
    this._timer = setInterval(() => {
      ...
      // Update display uniquement (pas tout re-render)
  ...
  _phaseSuivante() {
    ...
    if (this._phase === 'travail') {
      ...
      if (suivantExo < totalEx) {
        // Exercice suivant dans le même round
        if (config.repos > 0) {
          this._phase       = 'repos';
    ...
    player.innerHTML = `
      <div class="card mt-md"
        ...
        <div style="font-size:.88rem;color:var(--text-muted);
          ...
          ${config.rounds} rounds · ${config.exercices.length} exercices
          <br>+150 XP gagnés !
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;
                    gap:var(--space-sm)">
          <button onclick="Circuit._demarrerPlayer(Circuit._config||{})"
                  class="btn-secondary">
            🔄 Recommencer
          </button>
...
console.log('✅ Circuit.js v1.0 chargé');
